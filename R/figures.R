...
plot_assoc_stack <- function(data, corr, x.min, x.max, top.marker){
  ...
  if(geomtext){marker.plot <- marker.plot + geom_text(data=lead_marker, aes(x=label_pos,y=mlog10p,label=marker), vjust=-1, hjust=0.5, size=4)}else{marker.plot <- marker.plot + geom_label(data=lead_marker, aes(x=label_pos,y=mlog10p,label=marker), label.r=unit(0, "lines"), nudge_y=(-0.095*ylim), size=4, alpha=1)}
  return(marker.plot)
}
